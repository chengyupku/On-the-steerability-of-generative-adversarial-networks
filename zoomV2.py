# -*- coding: utf-8 -*-
"""
Created on Fri Jan 10 15:18:26 2020
...
# input placeholders
Nsliders = 1
z = tf.compat.v1.placeholder(tf.float32, shape=(None, dim_z))
y = tf.compat.v1.placeholder(tf.float32, shape=(None, vocab_size))
truncation = tf.compat.v1.placeholder(tf.float32, shape=None)
...
mask = tf.compat.v1.placeholder(tf.float32, shape=(
    None, img_size, img_size, num_channels))

# set walk parameters
alpha = tf.compat.v1.placeholder(tf.float32, shape=(None, Nsliders))
...
# losses
loss = tf.compat.v1.losses.compute_weighted_loss(tf.square(
...
train_step = tf.compat.v1.train.AdamOptimizer(lr).minimize(
    loss, var_list=tf.compat.v1.trainable_variables(scope=None), name='AdamOpter')
initializer = tf.compat.v1.global_variables_initializer()
config = tf.compat.v1.ConfigProto(log_device_placement=False)
config.gpu_options.allow_growth = True
sess = tf.compat.v1.Session(config=config)
sess.run(initializer)
saver = tf.compat.v1.train.Saver(tf.compat.v1.trainable_variables(scope=None))
